  ...
  <div>
    ...
    <transition name="up">
      ...
      >
        <!-- 标题 -->
        <div
          class="text-lg font-bold text-center text-zinc-900 dark:text-zinc-200 mb-2"
          v-if="title"
        >
          {{ title }}
        </div>
        ...
        <div class="flex justify-end" v-if="cancelHandler || confirmHandler">
          <m-button type="info" class="mr-2" :isActiveAnim="false" @click="onCancelClick">{{
            cancelText
          }}</m-button>
          <m-button type="primary" :isActiveAnim="false" @click="onConfirmClick">{{
            confirmText
          }}</m-button>
        </div>
      </div>
